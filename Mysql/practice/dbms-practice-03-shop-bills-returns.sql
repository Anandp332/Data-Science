...
[subject - [DB17-SHOP-RETURNS]]
[deadline - 23/08/2017 11am]
[attachment - a single file [rollno].sql]
[assignment text - [


consider the shop bills problem discussed in the lecture on 18/08/2017, now with the return of sold products.

a. write sql DDL statements to create the desired schema.

b. write sql DML statements to insert at least the following records
 b1. 5 products with 2 different prices on different dates
 b2. 3 orders with 3, 4 and 5 order details with different prices
 b3. 3 returns with 2 and 3 products each

c. write sql DDL/DML statements to create views for following
 c1. get all orders with their total amounts after returns
 c2. get all orders with their total amounts on a given date
 c3. get product-wise total amount across all orders that has been returned to the customers
 c4. get order-wise total amount that has been returned to the customers
